% !TeX root = skript.tex
Ein Zufallsgraphen ist ein Modell einer Graphfamilie --- etwa wie eine Zufallsvariable ein Modell einer Zufallsgröße ist.
...
Der Fokus auf die Modellierung von beobachteten Netzwerk kam erst später hinzu.
...
stellen wir uns etwa einen Freundschaftsgraphen vor, in dem Knoten die Nutzer eines einem sozialen Netzwerk sind und Kanten eine Freundschaft anzeigen.
...
\begin{lemma}\label{lemma:erwartete_kanten_in_gnp}
    Die \aside{Erwartete Kantenanzahl $\expv{|V|}$ in \Gnp} erwartete Anzahl an Kanten in einem gerichteten \Gnp Graphen ist \begin{equation*} \expv{m} = p n^2. \qedhere \end{equation*}
\end{lemma}
...
Daher sagen wir, dass \Gnm und \Gnp mit $p=m/n^2$ asymptotische (d.h. für $n \to \infty$) äquivalent sind.
...
Hier \aside{ausgabensensitive Algorthmen} helfen \emph{ausgabensensitive Algorithmen} (output-sensitive algorithm), deren Laufzeit maßgeblich durch die Größe der Ausgabe bestimmt wird;
die meisten Generatoren, die wir betrachten werden, fallen genau in diese Kategorie.

Ausgabensensitive \aside{Kantenlisten} Algorithmen müssen jedoch eine Ausgabedatenstruktur nutzen, die sich an die Größe anpasst.
...
Reservoir Sampling ist eine allgemeine Technik, die es uns erlaubt $k$ Element aus einem Datenstrom zu ziehen.
