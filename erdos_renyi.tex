...
On der Regel ist der Parameter aus dem Kontext klar und wird daher in der Literatur oft nicht erwähnt (im folgenden Lemma wäre es $k$):
...
das ist aber zum Einen unkritisch (weil $|S|$ trotzdem halbiert wird und so die Dicht $k/N$ steigt) und zum Anderen, wie wir gleich sehen werden, für praktische Implementierungen unerheblich.
...
Für diese zeigten Erd\H{o}s und R\'enyi, bereits einige Schwellwerte an denen sog. Phasenübergänge stattfinden.
Für \emph{sehr} dünne Graphe mit $m(n) = o(\sqrt n)$, bewiesen sie etwa, dass diese wahrscheinlich nur isolierte Knoten und Kanten beinhalten.
An dieser Aussage, wollen wir uns eine Aussage ansehen, mit der wir die Wahrscheinlichkeit solch eines Ereignisses abschätzen können.

\begin{lemma}
    Sehr dünne $\Gnm$ Graphen mit $m = o(m)$ haben wahrscheinlich keine Knoten mit mehr als einer Kante.
\end{lemma}

\begin{proof}
    Sei $p(n,m)$ die Wahrscheinlichkeit, dass keine zwei Kanten einen gemeinsamen Endpunkt haben, zu zeigen.
    Hierzu vergleichen wir die Anzahl der möglichen Graphen mit der Anzahl der gewünschten Graphen, die diese erfüllen.
    ...
    Da keine zwei Kanten einen gemeinsamen Endpunkt haben sollen und Schleifen verboten ist, müssen wir also $2m$ \underline{verschiedene} Knoten ziehen!
    ...
    \begin{align}
        \frac{\overbrace{\binom{n}{2m}}^\text{$2m$ verschiedene Knoten aus $n$}}{
...
Hier werden wir einen Beweis sehen, der Unabhängigkeit aller Kanten ausnutzt.
Zu erst schauen wir uns aber einige wohl bekannte Phasenübergänge an.
Sei $\epsilon >0$ hinreichend klein.
Dann gilt:
\begin{itemize}
    \item Falls $np < 1 - \epsilon$ haben mit hoher Wahrscheinlichkeit alle Zusammenhangskomponent Größe $\Oh{\log n}$.
    \item Falls $np = 1$ hat die größte Zusammenhangskomponent mit hoher Wahrscheinlichkeit Größe $\Omega(n^{2/3})$.
...
wir führen eine Tiefensuche auf einem $\Gnp$ Graphen aus, wobei die Nachbarschaft durch Ziehen von Einträgen in der Adjazenzmatrix erkunden.
Dann zeigen wir, dass für $np < 1$ so wenig Einsen vorhanden sind, dass es sehr unwahrscheinlich ist, dass wir eine große Zusammenhangskomponent beobachten.
Für $np > 1$, hingegen, sehen wir ausreichend viele Einsen.

\subsection{Tiefensuche auf $\Gnp$}
Im Folgenden definieren wir eine klassische Tiefensuche (DFS) auf eine Weise, die für den folgenden Beweise hilfreich ist.
...
\begin{itemize}
    ...
    \item \underline Unbekannte Knoten wurde noch nicht gefunden.
\end{itemize}
...
Dann gegeben wir das erste $w$ zurück, dessen Bit gesetzt war (falls es existiert).
...
\begin{itemize}
    ...
    \item Alle Knoten in $S$ gehören zur selben Zusammenhangskomponent.
\end{itemize}
