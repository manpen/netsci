...
Allerdings entspricht jede potentielle Kante einem Element in $S = V \times V$ für das wir je $\Oh{1}$ Arbeit verrichten müssen;
...
\begin{algorithm}[H]
    ...
    \While{$|R| < k$}{
        $x \gets $ zufälliges Element aus $U$\;
        \If{$x \not\in R$}{
            $R \gets R \cup \set{x}$
        }
    }
...
\begin{proof}
    ...
    Mit Wahrscheinlichkeit $p_i = (N - (i - 1)) / N$ ziehen wir nun ein Element, dass noch nicht Teil von $R$ ist; anderenfalls versuchen wir es erneut.
    Es gilt also
