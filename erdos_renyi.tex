% !TeX root = skript.tex
Zufallsgraphen sind ein Modell einer Graphfamilie.
Sie beschreiben ein Ensemble von Graphen, das so entworfen sein kann, dass bestimmte Eigenschaften in den betrachteten Graphen verstärkt auftreten.
...
\section{Effizientes Ziehen von $G(n, p)$ Graphen}
...
Um \aside{Annahme: aus einfachen Verteilungen kann in kostanter Zeit gezogen werden.} die Laufzeit dieses Algorithmus (wie auf diverser weiteren) bestimmen zu können, treffen wir folgende Annahme.
...
\subsection{Ziehen mit zufällige Sprünge}
...
\begin{itemize}
    ...
    \item Falls nicht prüfen wir, ob $U < 0.36 = f(0) + f(1) = F(1)$ ist und geben dann den Wert $1$ zurück.
