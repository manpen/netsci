...
der Ausdruck ist also im Wesentlichen eine Multiplikation von zwei $L$-Bit Zahlen zu einer Zahl mit $2L$ Bit gefolgt von einem Shift um $L$ Bits.
...
\begin{algorithm}[H]
    ...
    \Return{$m \div 2^L$}
    \caption{Uniformes Ziehen fast ohne Division}
...
Mit ein paar Tricks können beide Schranken auf $\Oh{1}$ gedrückt werden.
...
Für alle praktischen Anwendungen, in denen $n$ linear zu einer Ressource (etwa Speicher oder Laufzeit) korrespondiert, können wir uns $\lgs(n)$ daher als Konstante von höchstens $4$ vorstellen.

\def\rng#1#2{\ensuremath{R^{(#1)}}_{#2}}
\def\wght{\ensuremath{\text{weight}}}
\def\wrng#1#2{\ensuremath{\wght(R^{(#1)}_{#2})}}

\bigskip

Als Eingabe erhalten wir erneut $n$ strikt-positive\footnote{Es ist trivial auch Gewicht $0$ zu unterstützen; wir verbieten es hier nur, um Randfälle zu vermeiden.} Gewichte $(w_1, \ldots, w_n) \in \mathbb Q`_{>0}^n$.
Dann sei $W = \sum_{i=1}^n w_i$ deren Summe.
Wir nehmen ein \textsc{Ram}-Maschinenmodel an, wobei Arithmetik auf Integern (oder Festkommadarstellung mit ausreichender Präzision) der Größenordnung $\Oh{W}$ in Konstantzeit möglich sind.
Unsere Aufgabe ist es eine Datenstruktur zu konstruieren, die folgende Operationen effizient unterstützt:
\begin{itemize}
    \item \emph{sample$()$}: Gibt ein zufälliges $X \in [n]$ zurück, wobei $\prob{X = k} = w_k / W$ ist.
    \item \emph{update$(i, \Delta)$}: Aktualisiert das Gewicht~$w_i$ auf $w_i \gets w_i + \Delta$ und passt die Datenstruktur inkl. $W$ an.
\end{itemize}
...
Die Datenstruktur soll nur Bäume mit Höhe $\Oh{\lgs(n)}$ umfassen und es kann passieren, dass alle Elemente in einem Baum zusammengefasst werden.
Im Schnitt sollten wir also pro Ebene $k$ Kinder auf $\Oh{\log k}$ Vorfahren verteilen --- dann folgt die Tiefe $\Oh{\lgs(n)}$ automatisch.
...
Da die Gewichte unskaliert sind, kann es negative, aber auch positive Intervallnummern geben.
Wir bezeichnen die Summe aller Gewichte in Intervall $\rng{\ell}{j}$ als $\wrng{\ell}{j}$.
Je nachdem wie viele Elemente~$m$ in ein Intervall~$\rng{\ell}{j}$ fallen, behandeln wir das Intervall:
...
In Level $\ell > 1$ fassen wir die inneren Knoten aus $\ell - 1$ in Intervalle $\rng{\ell}{j}$ zusammen und verfahren rekursiv analog zu Level~$\ell = 1$.
...
Die Datenstruktur hat daher $\Oh{n}$ Knoten.

Da es im Allgemeinen mehrere Wurzeln gibt, ist die Datenstruktur ein Wald, wobei jeder Baum über die Leveltabelle seiner Wurzel adressierbar ist.
Jede Leveltabelle~$T_\ell$ entspricht einer Hashtabelle, die Wurzeln $\rng{\ell}{j}$ mit Schlüssel $j$ speichert (wir können auch alle Leveltabellen in einer Hashtabelle zusammenfassen, wenn die Schlüssel auf $(\ell, j)$ erweitern).
Pro Leveltabelle speichern wir noch das Gesamtgewicht des Levels $\wght(T_j) = \sum_{j \in T_\ell} \wrng{\ell}{j}$.
Als \emph{Hack} speichern wir zudem $X_\ell = \sum_{j \in T_\ell} 2^j$, d.h. eine Binärzahl in Festkommadarstellung in der das $i$-te Bit genau dann gesetzt ist, wenn $i \in T_\ell$.
Aufgrund unserer Annahmen bzgl. des Maschinenmodells können wir $X_\ell$ in einem Wort darstellen und in Konstantzeit z.B. das höchstwertigste Bit finden.
...
\begin{enumerate}
    ...
    \item Ziehe ein $j \in T_\ell$ zufällig gewichtet nach $\wrng{\ell}{j}$.
          ...
          Da sich Gewicht jedes Mal grob halbiert, ergibt sich eine erwartet konstante Laufzeit.

    \item Innerhalb von $\rng{\ell}{j}$ wählen wir nun ein Element gewichtet nach dessen Gewicht mittels Rejection Sampling.
          ...
          Falls $\ell = 1$, geben wir das entsprechende Element aus, anderenfalls steigen wir rekursiv in das gewählte interne Intervall ab.
          Da $L = \Oh{\lgs(n)}$ ergeben sich höchstens $\Oh{\lgs(n)}$ Rekursionschritte.
\end{enumerate}
