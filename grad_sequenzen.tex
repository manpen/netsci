...
Im Folgenden verallgemeinern wird die sog. Verwerfungsmethode (rejection sampling), die wir bereits in einer sehr einfachen Ausprägung beim Ziehen ohne Zurücklegen und Generieren einfacher Graphen genutzt.
...
bei $\Gnm$ mussten wir bereits gezogene Kanten zurückweisen.
...
Nachdem Ablehnen ungewünschter Element erhielten wir dann die Zielverteilung (target distribution) automatisch.
...
OBdA nehmen wir an, dass $\Omega$ endlich ist und es sich somit um diskrete Verteilungen handelt; die Verwerfungsmethode funktioniert aber auch mit kontinuierlichen Verteilungen.
...
\end{align}
Wenn es nun ein $x \in \Omega$ mit $t(x) < p(x)$, muss es zwangsläufig ein $x' \in \Omega$ geben, s.d. $t(x') > p(x')$.
...
\begin{proof}
    ...
    \begin{align}
        \prob{\text{Gebe $x$ zurück}} & = \prob{\text{akzeptiere $x$}\ \middle|\ \text{schlage $x$ vor}}                  \\
                                      & = \prob{\text{schlage $x$ vor}} \prob{\text{akzeptiere $x$}}                      \\
                                      & = p(x) \cdot \frac{t(x)}{s p(x)} = \frac{1}{s} t(x) \label{eq:reject-accept-prob}
    \end{align}

    Beobachte, die Ergebnisse \emph{Gebe $x$ zurück} und \emph{Gebe $y$ zurück} für $x \ne y$ disjunkt sind,
    Da pro Runde genau ein Element vorgeschlagen wird und es nicht $x$ und $y$ gleichzeitig sein kann.
    Wir können also die Akzeptanzwahrscheinlichkeit über das Gesetz der totalen Wahrscheinlichkeit als Summe der bedingten Wahrscheinlichkeiten berechnen:
    \begin{align}
        \prob{\text{Akzeptiere}} & = \sum_{x \in \Omega} \prob{\text{akzeptiere $x$}\ \middle|\ \text{schlage $x$ vor}} \\
                                 & = \frac{1}{s} \underbrace{\sum_{x \in \Omega} t(x)}_{=1} = \frac{1}{s}
        \qedhere
    \end{align}
\end{proof}

Der Algorithmus wiederholt, solange bis zum ersten Mal akzeptiert wird.
Die Fehlschläge sind dabei unabhängig voneinander sind.

\begin{corollary}\label{cor:laufzeit-rejection-sampling}
    Die Anzahl an Runde in \cref{alg:rejection_sampling} ist geometrisch mit Erfolgswahrscheinlichkeit $\frac{1}{s}$ verteilt.
...
\begin{exercise}
    ...
    Wie beeinflusst die Wahl die Korrektheit und Effizienz des von \cref{alg:rejection_sampling}?
\end{exercise}
...
\emph{Dieses Kapitel basiert in großen Teilen auf \cite{DBLP:journals/tomacs/Lemire19}.}
...
Der Standardweg eine Zahl $X \in [0, 2^L)$ auf $[0, s)$ zu reduzieren, besteht darin $Y = X \mod s$ zu berechnen
--- allein auf GitHub findet sich das Pattern \texttt{rand() \%} über 470.000 mal.
Der einzige Grund: es ist schnell zu schreiben und garantiert $Y \in [0, s)$.
Es ist aber zum einen recht langsam, und auch nur dann uniform, wenn $s | 2^L$, d.h. $2^L \mod s = 0$ ist.
Dies lässt sich an einem Bild leicht erkennen.
Betrachte die Sequenz $0, \ldots, 2^L \mod s$:
...
\begin{align}
    ...
    \overbrace{0, 1, \ldots, (2^L \mod s){-}1}^\text{$2^L \mod s$ Werte},\
    \label{eq:s_bloecke_in_2l}
\end{align}

Jeder Wert in $[0, s)$ taucht also exakt $\lfloor 2^L / s \rfloor$ oder $\lceil 2^L /s \rceil$ oft auf.
Es handelt sich also genau dann um eine uniforme Verteilung, wenn $\lfloor 2^L / s \rfloor = \lceil 2^L /s \rceil\ \ \Leftrightarrow\ \ 2^L \mod s = 0$.
Wir formalisieren dies in folgendem Lemma:

\begin{lemma}\label{lem:gleichverteilt_in_ab}
    Seien $a < b$ die Grenzen des ganzzahligen Intervalls $[a, b)$ und $s \in \mathbb N_{>0}$.
    Dann existieren genau $(b - a) \div s$ verschiedene Ganzzahlen $x, y \in [a, b)$ mit $x \mod s = y \mod s$, genau dann, wenn $s$ die Intervalllänge $b-a$ teilt.
\end{lemma}
\begin{proof}[Beweis durch Bild]
\end{proof}

Für $s \ll 2^L$ kann der Unterschied verschmerzbar klein sein, für $s > 2^L / 2$ gibt es aber einige Elemente die mit doppelter Wahrscheinlichkeit von anderen zurück geliefert werden.
...
Wenn $X$ uniform ist, ist es die Aussage dann auch.
Allerdings ist für $s \ll 2^L$ die Rejectionrate exorbitant hoch; in Erwartung benötigen wir $2^L / s$ Versuche!

Besser ist es natürlich ein möglichst großes $k \ge 1$ zu wählen, s.d. $k s \le 2^L$, dann nur $X \ge ks$ zu verwerfen und sonst $X / k$ zurückzuliefern.
Die beste Wahl ist $k = 2^L \div s$.
Dieses Verfahren hat eine Akzeptanzwahrscheinlichkeit von
\begin{align}
    \frac{k \cdot s}{2^L} = 1 - \frac{2^L \mod s}{2^L} \ge 1/2,
\end{align}
und benötigt daher in Erwartung höchstens zwei Versuche.

Beobachte, dass es hierbei egal ist, ob wir $X \ge ks$ oder $X < (2^L - ks) = 2^L \mod s = (2^L -s) \mod s$ verwerfen.
...
\begin{algorithm}[H]
    ...
    \Return $x \mod s$\;
    \caption{OpenBSD Algorithmus zum Ziehen uniformer Zufallszahlen.}
\end{algorithm}
...
Die Korrektheit des Verfahrens lässt sich erneut bildlich verstehen.
Zunächst beobachte, dass $r \le x$ und somit $x - r \ge 0$.
\todo{Bild}
...
Wenn für $s \to 2^L$ die Rejection-Rate jedoch steigt, nimmt auch die Anzahl der Division zu und ist im Worst-Case gar unbeschränkt (eine WHP Schranke lässt sich mittels Chernoff zeigen).
...
\begin{align}
    0 \ \le \ (x \cdot s) \div 2^L = (x \cdot s)\  < \  s\quad \forall x \in [0, 2^L), s \in [0, 2^L)
\end{align}

Dabei implementieren wir $(x \cdot s) \div 2^L$ als $\texttt{shift-right}(x \cdots s, L)$; der Ausdruck ist also im Wesentlichen eine Multiplikation von zwei $L$-Bit Zahlen zu einer Zahl mit $2L$ gefolgt von einem Shift um $L$ Bits.
...
Durch die Division, transformieren wir alle Vielfachen von $s$ in $[0, 2^L)$ auf $0$, in $[2^L, 2\cdots2^L)$ auf $1$, und $[i 2^L, (i+1)\cdots2^L)$ auf $i$.
Im Allgemeinen teilt aber $s$ nicht $2^L$ (sonst wären wir schon fertig);
daher müssen wir $2^L \mod s$ Element aus dem Intervall herausschneiden.
Wir verwerfen daher für jedes $i$ die Elemente in $[i 2^L, i 2^L + (2^L \mod s))$ und akzeptieren $[ i 2^L + (2^L \mod s), (i+1)\cdots2^L)$.
...
\begin{algorithm}[H]
    ...
    $m \gets x \times s$\;
    $\ell \gets m \mod 2^L$\;
    \If(\tcc*{Abkürzung falls wir sicher Akzeptieren können}){$\ell < s$}{
    $t \gets (2^L -s) \mod s$\tcc*{$2^L \mod s = (2^L -s) \mod s$}
    \While{$\ell < t$}{
    $x \gets \text{zufällige Ganzzahl aus $[0, 2^L)$}$\;
    $m \gets x \times s$\;
    $\ell \gets m \mod 2^L$\;
...
Beobachte, dass $t = 2^L \mod s (2^L - s) \mod s < s$ ist und wir nur verwerfen, wenn $\ell < t$.
Das bedeutet aber insbesondere auch, dass wenn $\ell \ge s$ ist, wissen wir, dass $\ell$ nicht kleiner als $t$ sein kann.
Daher müssen wir $t$ erst gar nicht berechnen.
Der Algorithmus läuft nur mit Wahrscheinlichkeit $s / 2^L$ in den \texttt{true}-Block; mit der Gegenwahrscheinlichkeit $1 - s/2^L$ ist er also divisionsfrei.

Die Strategie Abkürzungen für sicheres Verwerfen/Akzeptieren zu konstruieren, um unnötige Berechnungen zu sparen, ist eine gängige Technik in effizieten Implementierung;
man findet sie z.B. auch