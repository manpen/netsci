...
Davon gibt es $T - (\nu / k)^{1/\beta} T$ viele!
Sei $X$ nun ein zum Zeitpunkt~$T$ uniform gewählter Knoten und $P(k) = \prob{\deg_T(X) \le k}$ die kumulative Verteilungsfunktion.
Dann gilt:
\begin{align}
    P(k) & = \prob{\deg_T(X) \le k} = \frac{T - (\nu / k)^{1/\beta} T}{n_T} = \frac{T - (\nu / k)^{1/\beta} T}{n_0 + T} \\
...
\begin{algorithm}[t]
	...
	\For{$1 \le i \le N$}{
		\For{$1 \le j \le \nu$}{
			$x \gets $ uniform zufällig aus $E[n_0 + \nu(i-1)]$\;
			E.\pushback{$x$}\;
			E.\pushback{$n_0 + i$}\;
...
Dann ziehen wir unabhängig zufällig für jeden der neuen Einträge $1, \ldots, p$ den Index~$s_i$, von dem gelesen werden soll.
