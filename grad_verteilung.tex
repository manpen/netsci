...
Bevor die $p$ Einträge berechnet werden, schreiben wir an die Stellen $E[\ell..\ell+p] \gets \bot$ einen besonderen Wert, der anzeigt, dass diese Zelle keine valide Kante enthält.
...
Pro \qq{Runde} des Fallback Algorithmus können wir also nur eine weitere Kante produzieren.
...
\begin{lemma}\label{lem:par_ba_einzelschritt}
    Für \aside{Es handelt sich hier um einen Spezialfall des Geburtstags\qq{paradox}} eine vorhandene Kantenliste $E$ mit $\ell = |E|$ können wir $p = \Oh{\sqrt{\ell}}$ Kanten parallel in erwarteter konstanter Zeit einfügen.
\end{lemma}
...
Jede Kante besteht aus einem \qq{deterministischen} Eintrag, nämlich dem Index des neuen Knotens, der sich aus dem Kantenindex ableiten lässt, und einem zufälligen Wert (hier weiß).
...
Nach der ersten Runde sind alle Kanten fertig, deren $s_i$ ungerade war; in diesem Fall konnten wir nämlich einfach die \qq{deterministischen} Hälfte einer Kante lesen.
